...
test('should convert body type of object to JSON by default', () =>
  api.post('/', { a: 1, b: 2 }, { use: [ctx => {
    expect(ctx.options.headers.get('content-type')).toBe('application/json');
...
test('should request with PUT method', () =>
  api.put('/', null, { use: [ctx => expect(ctx.options.method).toBe('PUT')] })
);

test('should request with PATCH method', () =>
  api.patch('/', null, { use: [ctx => expect(ctx.options.method).toBe('PATCH')] })
);

test('should request with DELETE method', () =>
  api.delete('/', undefined, { use: [ctx => expect(ctx.options.method).toBe('DELETE')] })
);

test('should request with HEAD method', () =>
  api.head('/', undefined, { use: [ctx => expect(ctx.options.method).toBe('HEAD')] })
);
